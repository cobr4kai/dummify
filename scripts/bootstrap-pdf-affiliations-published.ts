...
import { readBootstrapPdfAffiliationsConfig } from "../src/lib/bootstrap/pdf-affiliations";
import { backfillPdfAffiliationsForPublishedPapers } from "../src/lib/ingestion/service";

async function main() {
  const config = readBootstrapPdfAffiliationsConfig();

  if (!config) {
    console.log("Skipping one-time PDF affiliation backfill because it is not configured.");
    return;
  }

  const databaseUrl = process.env.DATABASE_URL;
  if (!databaseUrl) {
    throw new Error("DATABASE_URL is required for PDF affiliation bootstrap backfill.");
  }
  ...
  try {
    ...
    if (status === "completed") {
      console.log(
        `Skipping one-time PDF affiliation backfill; task ${config.taskKey} already completed.`,
      );
      return;
    }
    ...
    console.log("Running one-time PDF affiliation backfill for published papers.");
    ...
    console.log(
      `Completed one-time PDF affiliation backfill for ${result.updatedCount} of ${result.paperCount} papers.`,
    );
  } catch (error) {
    await client.execute({
      sql: `
        ...
        JSON.stringify({
          message:
            error instanceof Error
              ? error.message
              : "Unknown PDF affiliation bootstrap backfill error.",
        }),
      ],
    });
